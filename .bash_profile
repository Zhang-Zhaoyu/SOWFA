		...
		if [ -f ~/.bashrc ]; then	# if .bashrc exit then run it
			. ~/.bashrc
		fi
		...
		SOWFA-2.4.0()
		{
		  ...
		  #module list
		   ...
		   if [ -z "$FOAM_INST_DIR" ]; then
		      ...
		      #. /home/zzy/OpenFOAM/OpenFOAM-2.4.0/etc/config/unset.sh
		   fi
		   ...
		   export HDF5_DIR=usr/lib/x86_64-linux-gnu	#&&/bin					                               
                    	
		   
		   export LD_LIBRARY_PATH=$SOWFA_DIR/lib/linux64GccDPOpt:$LD_LIBRARY_PATH #$SOWFA_DIR/lib/$WM_OPTIONS:$OPENFAST_DIR/lib:$LD_LIBRARY_PATH
		   export PATH=$SOWFA_DIR/applications/bin/linux64GccDPOpt:$PATH		#$SOWFA_DIR/applications/bin/$WM_OPTIONS:$PATH
		}
